...
public class GetValidateCode extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 显示汉字验证码 // 设置备选汉字，剔除一些不雅的汉字 String base =
		 ...
		 * 备选汉字的长度 int length = base.length(); // 设定图像背景色(因为是做背景，所以偏淡)
		 * g.setColor(getRandColor(200, 250)); g.fillRect(0, 0, width, height); //
		 * 备选字体 String[] fontTypes = {"\u5b8b\u4f53", "\u65b0\u5b8b\u4f53",
		 * "\u9ed1\u4f53", "\u6977\u4f53", "\u96b6\u4e66"}; int fontTypesLength =
		 * fontTypes.length; // 在图片背景上增加噪点 g.setColor(getRandColor(160, 200));
		 * g.setFont(new Font("Times New Roman", Font.PLAIN, 14)); for (int i =
		 * 0; i < 6; i++) {
		 * g.drawString("*********************************************", 0, 5 *
		 * (i + 2)); }
		 * 
		 * String s = ""; for (int i = 0; i < 6; i++) { int start =
		 * random.nextInt(length); String rand = base.substring(start, start +
		 * 1); s += rand; //设置字体的颜色 g.setColor(getRandColor(10, 150)); //设置字体
		 * g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],
		 * Font.BOLD, 18 + random.nextInt(6))); //将此汉字画到图片上 g.drawString(rand,
