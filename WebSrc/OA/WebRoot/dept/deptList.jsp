...
<html>
  ...
  <head>
	...
	<script language="javascript">
		...
		{
			document.forms[0].action="deleteDept.do?DeleteSdId="+name;
			document.forms[0].deptId.value=name;
  ...
  <body>
	...
	<div id="content" style="font-size:13px;">
		<div style="color: #FFFFFF; padding-top: 6px;">
			...
			<strong style="margin-left: 9px;">当前位置->部门列表</strong>
		</div>
		<hr class="line"/>
		...
		<div class="right" id="rightPanel">
  		<form action="deptList.do" method="post">  
			...
			<thead style="text-align:center; font-size: large; color: black; background-color: #dbdbda; font-family: 方正姚体;">部门列表</thead>
			<tr>
			<td class="tableColumeHead">部门编号</td>
			<td class="tableColumeHead">部门名称 </td>
			<td class="tableColumeHead">部门电话</td>
			...
			<logic:present name="deptList">
			 <logic:iterate id="list" name="deptList" type="com.icss.hit.hibernate.vo.SysDept">
			<tr onmouseover="this.style.backgroundColor = '#ffffff';" onmouseout="this.style.backgroundColor = '#fefccc';">
			<td>${ list.sdId }</td>
			<td>${ list.sdName}</td>
			<td>${ list.sdTel }</td>
			<td>${ list.sdInfo }</td>
			<td><a href="<%=request.getContextPath() %>/toModifyDept.do?sdId=${list.sdId }">修改</a></td>
			<td><a href="#" onclick="deleteDept('${ list.sdId }')">删除</a></td>
			</tr>
			 </logic:iterate>
		 	 </logic:present>
		 	 <td colspan="6"><a href="<%=request.getContextPath() %>/newDept.do" class="link">添加新部门</a></td>
			</table> 
			<html:errors property="DeleteDeptExist"/>
			${ requestScope.page }
		</form>
