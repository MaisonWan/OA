...
<html>
  ...
  <body>
	...
	<script type="text/javascript">
		<!--
		d = new dTree('d');
		d.add(0,-1,'名片夹',"UserInfo.do",'浏览个人信息');
		
		d.add(1,0,'添加名片','newCard.do','','','./images/info.gif','./images/info.gif','./images/info.gif');
		d.add(2,0,'我的名片夹','CardType.do','','','./images/info.gif','./images/info.gif','./images/info.gif');
		d.add(3,0,'个人名片','selfCard.do','','','./images/info.gif','./images/info.gif','./images/info.gif');
		d.add(4,0,'共享名片夹','shareCard.do','','','./images/info.gif','./images/info.gif');
		<%
			...
			for( int i = 0; i < deptSize; i++ ){
				...
				deptCurrentNum = num;
				%>
					d.add(<%=num++%>,4,'<%=dept.getSdName()%>','','<%=dept.getSdName()%>','','./images/dept.gif','./images/dept.gif');
				<%
		...
		%>
						d.add(<%=num++%>,<%=deptCurrentNum%>,'<%=user.getSuUsername()%>','getShareCardById.do?userId=<%=user.getSuId()%>','<%=user.getSuUsername()%>的详细信息','','./images/stuff.gif');
 		<%			}// end if
 				}// end for userList
 			}// end for deptList
 		%>
