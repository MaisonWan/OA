...
<html>
  ...
  <div id="head">
	...
	<div id="content" style="font-size:13px;">
		...
		<br/>
		<p class="passage">来自<big class="sign">${ requestScope.userInfo.sysDept.sdName }</big>的<big class="sign">${ requestScope.userInfo.suUsername }</big></p>
		<p class="passage">您有<big class="sign">${ messageCount }</big>条<a href="inBox.do">未读留言</a></p>	 
		...
		<div class="right" id="rightPanel">
			...
			<logic:iterate id="workPlan" name="planList" type="com.icss.hit.hibernate.vo.Schedule">
				<tr onmouseover="this.style.backgroundColor = '#ffffff';" onmouseout="this.style.backgroundColor = '#fefccc';">
				 <td><a href="readPlan.do?flag=planList&schId=${ workPlan.schId }">${ workPlan.schTitle }</a> </td>
				 <td><a href="readPlan.do?flag=planList&schId=${ workPlan.schId }">${ workPlan.schContent }</a></td>
				 <td>${ workPlan.schBegintime }</td>
				 <td>${ workPlan.schEndtime }</td>
				 <td><c:if test="${ workPlan.sysUserBySuIdTo.suId == workPlan.sysUserBySuIdFrom.suId }">本人</c:if>
				  	 <c:if test="${ workPlan.sysUserBySuIdTo.suId != workPlan.sysUserBySuIdFrom.suId }">${ workPlan.sysUserBySuIdFrom.suUsername }</c:if>
				 </td>
				 <td><c:if test="${ workPlan.schRead == '0' }"><a href="readPlan.do?flag=planList&schId=${ workPlan.schId }">未读</a></c:if>
				  	 <c:if test="${ workPlan.schRead == '1' }">已读</c:if>
				 </td>
				 <td><c:if test="${ workPlan.sysUserBySuIdTo.suId == workPlan.sysUserBySuIdFrom.suId }"><a href="deletePlan.do?flag=planList&schId=${ workPlan.schId }">删除</a></c:if>
				  	 <c:if test="${ workPlan.sysUserBySuIdTo.suId != workPlan.sysUserBySuIdFrom.suId }">----</c:if>
				 </td>
				 <td><a href="completePlan.do?schId=${ workPlan.schId }">完成</a></td>
				</tr>
			</logic:iterate>
			</table>
		<p align="center"><a href="planListMore.do" class="link" >更多</a></p>
		</logic:present>
		<logic:notPresent name="planList">
